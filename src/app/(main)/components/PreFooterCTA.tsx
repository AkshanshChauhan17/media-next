...
export default function PreFooterCTA() {
  ...
  return (
    <section className="w-full flex flex-col items-center pt-8">
      <button
        onClick={scrollToTop}
        className="text-[#C58B24] flex items-center gap-1.5 font-bold text-sm md:text-[15px] mb-4 hover:text-[#0A4D28] transition-colors"
      >
        Move To Top <ChevronsUp className="w-4 h-4 md:w-5 md:h-5" strokeWidth={3} />
      </button>

      <div className="w-full bg-[#0A4D28] relative flex justify-center py-12 md:py-16 px-4 overflow-visible md:min-h-[300px]">
        ...
        <div className="max-w-[1000px] w-full flex flex-col items-center md:items-start lg:items-center relative z-10 lg:-ml-32">
          ...
          <div className="flex flex-col md:flex-row items-center gap-5 md:gap-8 mb-6 md:mb-8">
            ...
            <Link
              href="/contact"
              className="bg-[#C58B24] hover:bg-[#b07b1e] transition-colors text-white px-6 md:px-8 py-2.5 md:py-3 rounded shadow-sm font-bold text-sm md:text-[15px] tracking-wide"
            >
              CONTACT US
            </Link>
          </div>

          <div className="w-full h-px bg-white/20 mb-6 md:mb-8 max-w-[700px]"></div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6 text-white w-full max-w-[700px] justify-center md:justify-start lg:justify-center">
            ...
            <div className="flex items-center gap-4 md:gap-5">
              <a href="#" className="hover:bg-[#C58B24] hover:border-transparent transition-all bg-transparent border-2 border-white rounded-full p-1.5">
        ...
        <div className="hidden md:flex absolute bottom-0 right-[5%] lg:right-[15%] w-[220px] lg:w-[260px] h-[300px] bg-white/5 rounded-t-full items-center justify-center backdrop-blur-sm border-t border-x border-white/10 z-0">
          <User className="w-32 h-32 text-white/20" strokeWidth={1} />
        </div>
      </div>
    </section>
  );
